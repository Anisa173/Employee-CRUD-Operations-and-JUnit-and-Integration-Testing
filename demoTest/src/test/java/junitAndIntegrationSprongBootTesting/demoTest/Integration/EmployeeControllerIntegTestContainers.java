...
@Testcontainers
public class EmployeeControllerIntegTestContainers {
    @Autowired
    ...
    public void givenListOfEmployee_whenFindAllEmployees_thenReturnListOfEmployee() throws Exception {
        ...
        employee = Employee.builder().firstName("Anisa").lastName("Çela").email("celaanisa07@gmail.com").build();
        repository.save(employee);
        Employee employee1 = Employee.builder().firstName("Anisa").lastName("Çela").email("celaanisa07@outlook.com").build();
        ...
        response.andDo(print()).andExpect(jsonPath("$.size()", is(employeeList.size())));
    ...
    public void givenEmployeeRecord_whenUpdateEmployeeObject_thenReturnUpdatedEmployee() throws Exception {
        ...
        Employee employeeFetched = repository.findById(employeeSaved.getEmplId()).get();
